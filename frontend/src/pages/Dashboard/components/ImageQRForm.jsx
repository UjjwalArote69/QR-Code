...
const ImageQRForm = ({ onBack, onGenerated, onLiveUpdate }) => {
  ...
  const [previewUrl, setPreviewUrl] = useState(null); // Local preview for the UI
  const [imageUrl, setImageUrl] = useState(''); // Fallback external link
  ...
  useEffect(() => {
    if (onLiveUpdate) {
      // Show a dummy URL for live preview if file is selected
  ...
  const handleFileChange = (e) => {
    if (e.target.files && e.target.files[0]) {
      ...
      setPreviewUrl(URL.createObjectURL(selectedFile)); // Create local preview thumbnail
      setError(null);
      if (!title) setTitle(selectedFile.name.split('.')[0]); // Set default title
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (uploadMode === 'file') {
        ...
        formData.append('qrType', 'Images'); // Matching your static array name
    ...
    <div className="flex flex-col h-full relative">
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 bg-white dark:bg-slate-950 border-t border-slate-200 dark:border-slate-800 shadow-[0_-10px_30px_rgba(0,0,0,0.05)] flex justify-end">
        <button 
          ...
          disabled={loading || (uploadMode === 'file' && !file) || (uploadMode === 'url' && !imageUrl)}
