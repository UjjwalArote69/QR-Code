...
const CreateQRView = () => {
  ...
  const handleTypeSelect = (type) => {
    ...
    setLivePreview({ url: '', fgColor: '#000000', bgColor: '#ffffff' }); // Reset preview
  };
  ...
  const qrDisplayValue = generatedLink ? generatedLink : (livePreview.url || "https://nexusqr.com");
      ...
      <div className="flex-1 xl:overflow-y-auto bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
        {!selectedType ? (
          <div className="max-w-4xl mx-auto p-4 sm:p-8">
            <QRGridSection
              title="Dynamic QR" badge="with tracking" description="Update content in real time, without changing your code."
              items={dynamicQrTypes} onSelectType={handleTypeSelect}
            />
            <QRGridSection
              title="Static QR" badge="no tracking" description="Fixed content that cannot be changed once printed."
              items={staticQrTypes} onSelectType={handleTypeSelect}
            />
          </div>
        ) : (
          <div className="h-full">
            {selectedType.name === "Website" ? (
              <WebsiteQRForm 
                onBack={handleBackToGrid} 
                onGenerated={(link) => setGeneratedLink(link)} 
                onLiveUpdate={(data) => setLivePreview(data)} // Catching the live updates here!
              />
            ) : (
              <div className="p-8 text-center text-slate-500 flex flex-col items-center justify-center h-full">
                <p className="mb-4 text-lg">Form for <strong>{selectedType.name}</strong> is coming soon!</p>
                <button 
            ...
            {selectedType ? (
              <div className="animate-in fade-in zoom-in duration-300 flex flex-col items-center">
                ...
                <div 
                  ...
                  style={{ backgroundColor: livePreview.bgColor }}
                >
                  <QRCodeSVG
                    value={qrDisplayValue}
                    size={180}
                    level={"H"} 
                    fgColor={livePreview.fgColor}
                    bgColor={livePreview.bgColor}
                    includeMargin={false}
                  />
                </div>
                ...
                {generatedLink ? (
                  ...
                  <p className="text-xs text-slate-400 mt-4">
                    {livePreview.url ? "Updating live..." : "Enter a URL to update"}
                  </p>
                )}
              </div>
            ) : (
